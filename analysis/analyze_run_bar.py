...
gROOT.ProcessLine('sAnalysis.channels[0]=%d;'%channels[0])
gROOT.ProcessLine('sAnalysis.channels[1]=%d;'%channels[1])
gROOT.ProcessLine('sAnalysis.channels[2]=%d;'%channels[2])
gROOT.ProcessLine('sAnalysis.channels[3]=%d;'%channels[3])
#gROOT.ProcessLine('sAnalysis.refChId=%d;'%channels[0])
...
gROOT.ProcessLine('cAnalysis.channels[3]=%d;'%channels[3])
...
################################################
c1_energy_ref = TCanvas("c1_energy_ref", "", 900, 700)
c1_energy_refCoinc = TCanvas("c1_energy_refCoinc", "", 900, 700)
...
fitSpectrum(histos['h1_energy_ref'],fTot_ref,minEnergy,maxEnergy,c1_energy_ref,fitResults,"ref","",opt.run,opt.outputDir)
histos['h1_energy_ref'].Write()
## Bar
fTot_bar = TF1("fTot_bar",totalFunction,minEnergy,maxEnergy,n_paramameters)
fTot_bar.SetNpx(1000)
fitSpectrum(histos['h1_energyTot_bar'],fTot_bar,minEnergy,maxEnergy,c1_energy_refCoinc,fitResults,"bar",opt.barCode,opt.run,opt.outputDir)
...
fTot_ref_coinc = TF1("fTot_ref_coinc",totalFunction_coinc,minEnergy_coinc,maxEnergy_coinc,n_paramameters_coinc)
fTot_ref_coinc.SetNpx(1000)
fitSpectrum_coinc(histos['h1_energy_ref_coinc'],fTot_ref_coinc,minEnergy_coinc,maxEnergy_coinc,c1_energy_bar,fitResults,"refCoinc","",opt.run,opt.outputDir)
histos['h1_energy_ref_coinc'].Write()
...
## Ref (ref)
fExpo_ref = TF1("fExpo_ref","[0]*(1-TMath::Exp(-[1]*x))",minE,maxE)
fitSaturation(fExpo_ref,minE,maxE,c1_sat_ref,fitResults,"ref")
...
print fitResults[('refCoinc',"peak1","mean","value")] - fitResults[('refCoinc',"peak1","sigma","value")]
print fitResults[('refCoinc',"peak1","mean","value")] + fitResults[('refCoinc',"peak1","sigma","value")]
...
gROOT.ProcessLine("ctrAnalysisBar_new ctAnalysis(treeCTR);")
...
gROOT.ProcessLine('ctAnalysis.channels[3]=%d;'%channels[3])
gROOT.ProcessLine('ctAnalysis.outputFile="%s";'%(opt.outputDir+"/"+"histo_Run"+run+"_BAR"+str(str(opt.barCode).zfill(6))+".root"))
gROOT.ProcessLine('ctAnalysis.ref_511Peak_mean=%f;'%fitResults[('refCoinc',"peak1","mean","value")])
gROOT.ProcessLine('ctAnalysis.ref_511Peak_sigma=%f;'%fitResults[('refCoinc',"peak1","sigma","value")])
...
histos['h1_CTR'].GetXaxis().SetTitle("t_{bar} - t_{ref} [ps]")
...
f_res.SetParLimits(1,1000,50000)
f_res.SetParLimits(2,0,500)
f_res.SetParameter(0,3000)
f_res.SetParameter(1,20000)
f_res.SetParameter(2,20)
h2.Fit(f_res,"Q0")
h2.Fit(f_res,"+")
...
c1_tDiff.cd()
c1_tDiff.Update()
c1_tDiff.SaveAs(opt.outputDir+"/"+"Run"+str(opt.run).zfill(6)+"_BAR"+str(opt.barCode).zfill(6)+"_tDiff"+".pdf")
c1_tDiff.SaveAs(opt.outputDir+"/"+"Run"+str(opt.run).zfill(6)+"_BAR"+str(opt.barCode).zfill(6)+"_tDiff"+".png")
c1_tDiff.Write()
h2.Write()

histos['h2_tDiffRefVsRefEnergy'].FitSlicesY()
h2=gDirectory.FindObject('h2_tDiffRefVsRefEnergy_2')
h2.Fit(f_res,"Q0")
h2.Fit(f_res,"+")
...
pt3 = TPaveText(0.100223,0.915556,0.613586,0.967407,"brNDC")
text3 = pt3.AddText( "Run" + str(opt.run).zfill(6) + " REF" )
pt3.SetFillColor(0)
pt3.Draw()
#FIXME: check why it does not show the label on the canvas!
...
c1_tDiff.cd()
c1_tDiff.Update()
c1_tDiff.SaveAs(opt.outputDir+"/"+"Run"+str(opt.run).zfill(6)+"_BAR"+str(opt.barCode).zfill(6)+"_tDiffRef"+".pdf")
c1_tDiff.SaveAs(opt.outputDir+"/"+"Run"+str(opt.run).zfill(6)+"_BAR"+str(opt.barCode).zfill(6)+"_tDiffRef"+".png")
c1_tDiff.Write()
h2.Write()
...
for ch in channels:
    ...
    for tac in range(0,4):
        histosVsTot_Ped1[(ch,tac)].Write()
        histosVsTot_Ped2[(ch,tac)].Write()
        histosVsTot_PedTot[(ch,tac)].Write()
...
#Ref
print "--- Ref ---"
print "Ref Peak 1: "+str(fitResults[('ref',"peak1","mean","value")])+" +/- "+str(fitResults[('ref',"peak1","mean","sigma")]) 
print "Ref Peak 2: "+str(fitResults[('ref',"peak2","mean","value")])+" +/- "+str(fitResults[('ref',"peak2","mean","sigma")]) 
print "Ref Backpeak : "+str(fitResults[('ref',"backpeak","mean","value")])+" +/- "+str(fitResults[('ref',"backpeak","mean","sigma")]) 
print "Ref Peak 1 Coinc: "+str(fitResults[('refCoinc',"peak1","mean","value")])+" +/- "+str(fitResults[('refCoinc',"peak1","mean","sigma")]) 
#print "Ref Alpha: "+str(fitResults[('ref',"peak12","alpha","value")])+" +/- "+str(fitResults[('ref',"peak12","alpha","sigma")]) 
#print "Ref Beta: "+str(fitResults[('ref',"peak12","beta","value")])+" +/- "+str(fitResults[('ref',"peak12","beta","sigma")]) 
...
#print "Ref Alpha: "+str(fitResults[('bar',"peak12","alpha","value")])+" +/- "+str(fitResults[('bar',"peak12","alpha","sigma")]) 
#print "Ref Beta: "+str(fitResults[('bar',"peak12","beta","value")])+" +/- "+str(fitResults[('bar',"peak12","beta","sigma")]) 
...
print 
#print "----"
print "xtal:{0} RUN:{1} ly_bar:{2:.2f} Eres_bar:{3:.1f}% ctr_sigma:({4:.2f}+/-{5:.2f})ps ctr_mean:{6:.2f}ps stoc:({7:.2f}+/-{8:.2f})ps -- ly_ref:{9:.2f} Eres_ref:{10:.1f}%".format(
    opt.barCode,
    opt.run,
    ...
    fitResults[('refCoinc',"peak1","mean","value")],
    100*fitResults[('refCoinc',"peak1","sigma","value")]/fitResults[('refCoinc',"peak1","mean","value")]   
)
...
peak2_mean_ref = array( 'd', [ -999. ] )
err_peak2_mean_ref = array( 'd', [ -999. ] )
peak1_sigma_ref = array( 'd', [ -999. ] )
err_peak1_sigma_ref = array( 'd', [ -999. ] )
peak2_sigma_ref = array( 'd', [ -999. ] )
err_peak2_sigma_ref = array( 'd', [ -999. ] )
#alpha_ref = array( 'd', [ -999. ] )
#err_alpha_ref = array( 'd', [ -999. ] )
#beta_ref = array( 'd', [ -999. ] )
#err_beta_ref = array( 'd', [ -999. ] )
peak1_mean_refCoinc = array( 'd', [ -999. ] )
err_peak1_mean_refCoinc = array( 'd', [ -999. ] )
peak1_sigma_refCoinc = array( 'd', [ -999. ] )
err_peak1_sigma_refCoinc = array( 'd', [ -999. ] )
#
...
treeOutput.Branch( 'peak2_mean_ref', peak2_mean_ref, 'peak2_mean_ref/D' )
treeOutput.Branch( 'err_peak2_mean_ref', err_peak2_mean_ref, 'err_peak2_mean_ref/D' )
...
treeOutput.Branch( 'peak2_sigma_ref', peak2_sigma_ref, 'peak2_sigma_ref/D' )
treeOutput.Branch( 'err_peak2_sigma_ref', err_peak2_sigma_ref, 'err_peak2_sigma_ref/D' )
#treeOutput.Branch( 'alpha_ref', alpha_ref, 'alpha_ref/D' )
#treeOutput.Branch( 'err_alpha_ref', err_alpha_ref, 'err_alpha_ref/D' )
#treeOutput.Branch( 'beta_ref', beta_ref, 'beta_ref/D' )
#treeOutput.Branch( 'err_beta_ref', err_beta_ref, 'err_beta_ref/D' )
treeOutput.Branch( 'peak1_mean_refCoinc', peak1_mean_refCoinc, 'peak1_mean_refCoinc/D' )
treeOutput.Branch( 'err_peak1_mean_refCoinc', err_peak1_mean_refCoinc, 'err_peak1_mean_refCoinc/D' )
treeOutput.Branch( 'peak1_sigma_refCoinc', peak1_sigma_refCoinc, 'peak1_sigma_refCoinc/D' )
treeOutput.Branch( 'err_peak1_sigma_refCoinc', err_peak1_sigma_refCoinc, 'err_peak1_sigma_refCoinc/D' )
#
...
peak2_mean_ref[0] = fitResults[('ref',"peak2","mean","value")]
err_peak2_mean_ref[0] = fitResults[('ref',"peak2","mean","sigma")]
peak1_sigma_ref[0] = fitResults[('ref',"peak1","sigma","value")]
err_peak1_sigma_ref[0] = fitResults[('ref',"peak1","sigma","sigma")]
peak2_sigma_ref[0] = fitResults[('ref',"peak2","sigma","value")]
err_peak2_sigma_ref[0] = fitResults[('ref',"peak2","sigma","sigma")]
#alpha_ref[0] = fitResults[('ref',"peak12","alpha","value")]
#err_alpha_ref[0] = fitResults[('ref',"peak12","alpha","sigma")]
#beta_ref[0] = fitResults[('ref',"peak12","beta","value")]
#err_beta_ref[0] = fitResults[('ref',"peak12","beta","sigma")]
peak1_mean_refCoinc[0] = fitResults[('refCoinc',"peak1","mean","value")]
err_peak1_mean_refCoinc[0] = fitResults[('refCoinc',"peak1","mean","sigma")]
peak1_sigma_refCoinc[0] = fitResults[('refCoinc',"peak1","sigma","value")]
err_peak1_sigma_refCoinc[0] = fitResults[('refCoinc',"peak1","sigma","sigma")]
#
